...
import { extractBreadcrumbs, SignUpFormData, SignUpSchema } from '@/utils';
...
function SignUpPageContent() {
  ...
  const {
    ...
    resolver: effectSchemaResolver(SignUpSchema),
    mode: 'onBlur',
  ...
  const onSubmit = async (data: SignUpFormData) => {
    ...
    try {
      ...
      showToast({
        title: TOAST_MESSAGES.SIGN_UP.ERROR.title,
        description:
          getFriendlyMessage(error) || TOAST_MESSAGES.SIGN_UP.ERROR.description,
        variant: TOAST_VARIANTS.ERROR,
        duration: 5000,
      });
      setIsLoading(false);
