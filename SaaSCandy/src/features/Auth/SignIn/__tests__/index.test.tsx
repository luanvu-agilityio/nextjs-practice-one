...
// Default social auth mock should return a resolved Promise
mockHandleSocialAuth.mockImplementation(() => Promise.resolve({}));
...
describe('SignInPageContent - Interactive', () => {
  ...
  it('handles submit and shows error on failed code send', async () => {
    mockSend2FACode.mockImplementationOnce(() =>
      Effect.succeed({ success: false, error: 'Invalid credentials' })
    );
    render(<SignInPageContent />);
    await userEvent.click(screen.getByText('Submit'));
  });
...
describe('SignInPageContent - Additional Coverage', () => {
  ...
  it('shows error toast on failed verification', async () => {
    mockVerify2FACode.mockImplementationOnce(() =>
      Effect.succeed({ success: false, error: 'Invalid code', data: false })
    );
    render(<SignInPageContent />);
    await userEvent.click(screen.getByText('Submit'));
    await waitFor(() => {
  ...
  it('handles social sign in error', async () => {
    mockHandleSocialAuth.mockImplementationOnce(() =>
      Promise.reject(new Error('Social error'))
    );
    render(<SignInPageContent />);
    await userEvent.click(
      screen.getByRole('button', { name: /sign in with google/i })
