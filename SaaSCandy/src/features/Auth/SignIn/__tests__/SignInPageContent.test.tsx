...
// TwoFactorForm and Sms2FAForm are imported directly by the SignInPageContent
// component (from separate files). Ensure they are mocked as well so our
// captured handler approach works and we don't render the real implementations
// during tests.
...
// Provide a safe getSession mock so HttpClient.getAuthHeader won't throw when
// tests don't care about session contents. Also export signIn.email for the
...
describe('SignInPageContent', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockSend2FACode.mockImplementation(() => Effect.succeed({ success: true }));
    mockVerify2FACode.mockImplementation(() =>
      Effect.succeed({ success: true, data: true })
    );
    mockSignInEmail.mockResolvedValue({ error: undefined });
    // Default social auth mock returns an Effect
    mockHandleSocialAuth.mockImplementation(() => Effect.succeed(undefined));
  ...
  it('shows error toast when signIn.email fails after verification', async () => {
    ...
    // Force the component into the 2FA verification state to make the test deterministic
    const useStateSpy = jest.spyOn(React, 'useState') as jest.SpyInstance;
    const dispatchStub = (() => undefined) as React.Dispatch<
      React.SetStateAction<unknown>
    >;
    ...
    // Now the TwoFactorForm should be rendered; click verify
    await waitFor(() =>
  ...
  it('handles social sign-in error', async () => {
    mockHandleSocialAuth.mockImplementationOnce(() =>
      Effect.fail(new Error('social-fail'))
    );
    render(<SignInPageContent />);
